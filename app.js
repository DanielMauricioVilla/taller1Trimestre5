...
btnCalculate.addEventListener ('click', () => {
    if (NoteStudent1.value >= 0 && NoteStudent1.value <= 5 && NoteStudent2.value >= 0 && NoteStudent2.value <=5 && NoteStudent3.value >= 0 && NoteStudent3.value <=5 ){     
        notes ();
    }else {
        alert("Nota no valida, Verifique las notas ingresadas")
    }
})
...
function notes (){
    ...
    noteFinal = ((note1 )+(note2 )+(note3 )).toFixed(1);
    ...
    // }else {
    //     result.textContent = "Nota no valida"
    // }
}
